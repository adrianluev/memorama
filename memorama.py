...
car = path('car.gif') # car guarda el path a la memoria donde se encuentra car.gif

tiles = list(range(32)) * 2 # se genera una lista de 64 items que van del 1-32 dos veces
state = {'mark': None}
hide = [True] * 64
...
def tap(x, y): #cuenta los taps y ve los cuadros escondidos
    ...
    spot = index(x, y)
    mark = state['mark']
    if mark is None or mark == spot or tiles[mark] != tiles[spot]: #Hace un update para ver que tile clickeo el usuario
        state['mark'] = spot
    else:
        hide[spot] = False
        hide[mark] = False
...
def draw():
    ...
    if(escondidas == 0 ):
        ...
        goto(-200,150)
        color('black')
        write('¡HAZ GANADO UN CARRO!', font=('Arial',20,'normal')) #pone el mensaje felicitatorio
        up()
        goto(-200,130)
        color('black')
        write(f'Hiciste {taps} taps', font=('Arial',15,'normal')) #escribe el numero de taps que hace el usuario
        nombre()
